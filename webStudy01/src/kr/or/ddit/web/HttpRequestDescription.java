 ...
 *  DELETE : 삭제 (D)
 *  TRACE : 디버깅
 *  OPTIONS : ex) tttp method 의 지원여부 확인
 *  HEAD : 응답데이터를 body 가 없는 형태로 받는다.
 * Request Head : 클라이언트와 요청에 관한 부가 정보(metadate)
 * 	헤더명 : 헤더값...
 ...
 *  1) 파라미터 확보(바디) : 기본적으로 모든 파라미터는 문자열의 형태로 전송됨.
 *  	String getParameter(name) : 하나의 파라미터 명으로 하나의 값이 전송
