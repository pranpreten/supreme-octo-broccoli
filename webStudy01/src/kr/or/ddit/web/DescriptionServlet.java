 ...
 * 
 * 서블릿 개발 단계
 * 1. HttpServlet 을 상속하여 구현.
 * 2. 컴파일 -> 웹 어플리케이션의 classpath에 배포(/WEB-INF/classes)
 * 3. 등록 
 * 	  1) web.xml 을 이용한 명시적 등록
 * 	  2) @webservlet 을 이용한 등록 (어노테이션, since 3.0)
 ...
 * : 서블릿의 생명주기를 관리하는 역할
 * 	 (서블릿의 생명주기 내에서 특정 이벤트가 발생시 해당 콜백 메서드를 호출)
 * : 싱글턴, 요청이 쓰레드로 처리되며, 해당 쓰레드는 풀링 정책에 따라 관리됨.
 * 
 * 서블릿의 생명주기와 관련된 콜백 메서드의 종류
 * - init : 서블릿의 객체가 생성된 직후에 호출되는 메서드
 * - service : 요청이 발생할 때 마다 반복 호출
 *           요청의 HTTP 메서드에 따라 doXXX 계열 메서드를 호출.
 * - doGet 등의 doXXX 계열 메서드
 *         : 요청이 발생될때마다 반복 호출
 *           HTTP 메서드에 따라 service에 의해 호출됨.
 * - destroy : 서블릿의 객체가 소멸되기 직전에 호출되는 메서드
 * 
 * WAS의 서블릿을 이용한 요청 처리 단계
 * 1) 서블릿 매핑 정보를 통해 현재 요청을 처리할 서블릿을 검색.
 * 2) 해당 서블릿의 객체 생성 여부를 판단
 * 3) 현재 요청이 최초의 요청이라면(객채 생성 전), 해당 서블릿의 객체를 생성 -> init 호출
