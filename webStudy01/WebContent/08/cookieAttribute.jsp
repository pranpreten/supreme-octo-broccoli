...
<h4> 쿠키의 속성들 </h4>
<pre>
	1. path : 브라우저의 쿠키 저장소에 저장된 쿠키가 다시 서버로 재전송 딜지 여부를 결정.
	...
	3. maxAge : 만료 시간, 생략하면 기본값으로 세션의 만료사간이 반영됨
			-1 : 브라우저의 종료 시점에 삭제.
	...
	<%
		...
		Cookie allPathCookie = new Cookie("allPathCookie",URLEncoder.encode("모든경로에서 사용할 쿠키", "UTF-8"));
		allPathCookie.setPath(request.getContextPath());
		response.addCookie(allPathCookie);
		
		Cookie otherPathCookie = new Cookie("otherPathCookie",URLEncoder.encode("09의 하위에서 사용할 쿠키", "UTF-8"));
		...
		otherDomainCookie.setDomain(".lkj.com");
